...
void ofApp::update() {
    ...
    while (receiver.hasWaitingMessages()) {
        ...
        if (draw_args.size()) draw_args.clear();
        if (r_msg.size() < line_num) {
            r_msg.push_back("");
        }
    ...
    if (send_bang) {
        ...
        cout << " -------------- " << endl;
        if (send_bang) send_bang = false;
    }
    
    if (r_args.size()) {
        if (line_index >= line_num) {
            r_msg.erase(r_msg.begin());
            line_index--;
        ...
        for (int i = 0; i < r_args.size(); i++) {
            r_msg[line_index] += r_args[i];
            r_msg[line_index] += " ";
        }
        if (line_index < line_num) {
            line_index++;
            cout << "line_index 2 : " << line_index << endl;
        }
    }
}
...
void ofApp::drawMonitor(ofEventArgs& args) {
    for (int i = 0; i <= line_num / 2; i++) {
        ofSetColor(100);
        ofDrawRectangle(0, i * 36, 300, i * 36 + 18);
        ofSetColor(80);
        ofDrawRectangle(0, i * 36 + 18, 300, i * 36 + 36);
    }
    
    for (int i = 0; i < line_num; i++) {
        if (i < r_msg.size()) {
            ofSetColor(255);
            ofDrawBitmapString(r_msg[i], 10, 14 + i * 18);
        }
    }
    
    bool reset = false;
    if (ofGetMouseY() > 270 && ofGetMouseY() < 300) {
        if (ofGetMousePressed()) {
            reset = true;
            if (r_msg.size()) r_msg.clear();
        }
        ofSetColor(120);
        ofDrawRectangle(0, 270, 300, 300);
    }
    ...
    ofDrawBitmapString("clear", 130, 290);
}
